...
#REPORTE DE HOGARES POR COMUNIDAD
data1<-gt_emory_data_arm1 %>% select("id_tamizaje"=id, "id_estudio"=s4_main_id, "fecha_enrolamiento"=s4_date) %>% filter(!is.na(id_estudio)) %>% 
  left_join(
    gt_emory_data_arm2 %>% select("id_estudio"=id, "fecha_tamizaje"=s6_date, s6_arm) %>% filter(!is.na(fecha_tamizaje))
            ) %>% left_join(
      gt_emory_data_arm1 %>% select("id_tamizaje"=id, m17_date, "fpp"=m17_ga) %>% filter(!is.na(m17_date))
                ) %>% left_join(
    gt_participants %>% 
    mutate(
      id_estudio = if_else(
        condition = grepl("^3[35]", record_id),
        true = record_id,
        false = id_estudio
      )
    ) %>% select(id_estudio, com_jalapa, lat, long)
  ) %>% mutate(
    community = recode(
      com_jalapa,
      "2101308" = "SANTA CRUZ",
    ...
    #salidas %>% select("id_estudio"=id, sale)
  #) %>% 
  #filter(is.na(sale)) %>% filter(community=="LA TOMA" | community=="LOS LOPEZ" | community=="EL MIRADOR" ) %>% 
  left_join(
    gt_emory_data_arm2 %>% select("id_estudio"=id, c30_date, c30_age) %>% 
  filter(!is.na(c30_date)) %>% filter(c30_age<"37")
) %>% mutate(
  fecha_nacimiento=if_else(is.na(c30_date),fpp,c30_date)
) %>% mutate(fecha_sale=fecha_nacimiento + lubridate::days(365)) %>% write_csv("output/salidas.csv")

data1 %>% select(
  id_estudio,id_tamizaje,community, fecha_sale
) %>% mutate(
  mes=lubridate::month(fecha_sale, label=FALSE),
  anio=lubridate::year(fecha_sale)
) %>% group_by(community,mes, anio) %>% summarize(
  cantidad=n()
) %>% 
split(.$community) %>% 
  walk(
    ...
    ggsave(
      filename = paste0("output/salidas/",first(.x$community),".pdf"),plot = plot1
    )
    } )
    

#copia de codigo de alex
total_hogares<-data1 %>% select(
  id_estudio,id_tamizaje,community, fecha_sale
) %>% mutate(
  mes=as.character(fecha_sale, format="%B"),
  anio=lubridate::year(fecha_sale),
  mes_salida=paste0(mes,"-",anio)
) %>% 
  group_by(community,mes_salida) %>% 
  summarize(
    cantidad=n()
  ) %>% group_by(community) %>% summarize(
...
data1 %>% select(
  id_estudio,id_tamizaje,community, fecha_sale
          ...
          mes=as.character(fecha_sale, format="%m"),
          anio=lubridate::year(fecha_sale),
          mes_salida=paste0(mes,"-",anio)
        ) %>% 
  group_by(community,mes_salida) %>% 
    summarize(
          cantidad=n()
          ) %>% group_by(community) %>% mutate(correlativo=1:n()) %>%  left_join(
            total_hogares
          ) %>% mutate(primera_resta=total_hogares-cantidad) %>% write_csv("output/hogares.csv")

hogares_comunidad<-read_csv("output/hogares.csv")

hogares_comunidad %>% mutate(
  salida=as.character(mes_salida, format="%m-%y")
  ) %>% 
  split(.$community) %>% 
  walk(
    ~ {plot1<-.x %>% ggplot(
        aes(x=mes_salida, y=saldo)
      )+ geom_point()+
      geom_text_repel(aes(label=paste0("sale: ",cantidad)), size = 2)
    ggsave(
      filename = paste0("output/salidas/",first(.x$community),".pdf"),plot = plot1  + ggtitle(paste0("Total de Hogares participando:",.x$total_hogares)) + xlab("Mes de salida")+ ylab("Hogares restantes")
    )
    } )
...
plot1<-data1 %>% select(
  ...
  cantidad=n()
) %>% ggplot( aes(x=community, y=cantidad))+ geom_point()
ggsave("grafica.png", plot= plot1 + ggtitle("HOLA")+ xlab("casititas") + ylab("casototas"))
