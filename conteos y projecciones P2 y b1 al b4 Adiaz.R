...
#---------------------------------------------------------------------
comunidades<-gt_participants %>% select(record_id,id_estudio, com_jalapa) %>%
...
#calendario visitas P2 de Junio a Diciembre
gt_emory_data %>%filter(!is.na(s4_main_id)) %>% select("id_tamizaje"=id, "id"=s4_main_id) %>% 
  left_join(
    gt_emory_data %>% filter(!is.na(m17_date)) %>% select("id_tamizaje"=id, m17_ga)
  ) %>% anti_join(
    bind_rows(
      gt_emory_data %>%
    ...
    gt_emory_data %>% filter(!is.na(c30_date)) %>% select(id, c30_date)
  ) %>% left_join(
    gt_emory_data %>% filter(!is.na(c30a_date)) %>% select(id, c30a_date)
  ) %>% mutate(
    fec=if_else(is.na(c30_date), c30a_date, c30_date),
    nacido=if_else(is.na(fec),"No","Si"),
    fec_nacimiento=if_else(is.na(fec),m17_ga, fec)
  ) %>% mutate(
    fec_concepcion=fec_nacimiento - lubridate::days(280),
    inicia_p2=fec_concepcion + lubridate::days(224),
    termina_p2=fec_concepcion + lubridate::days(252),
    f_b1=fec_nacimiento + lubridate::days(30*3),
    f_b2=fec_nacimiento + lubridate::days(30*6),
    f_b3=fec_nacimiento + lubridate::days(30*9),
    f_b4=fec_nacimiento + lubridate::days(30*12)
  ) %>% write_csv("output/visitas_b1_b4.csv")
  #write_csv("output/les_toca_p2.csv")
