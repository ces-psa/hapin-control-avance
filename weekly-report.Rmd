...
ga_fcf <- gt_emory_data %>%
  ...
  mutate(
    tipo = case_when(
      s4_consent == 1 ~ "Inscrita en HAPIN",
      TRUE ~ "Solo tamizada"
    ),
    fpp = m17_ga,
    fcf = m17_hr_bpm,
  ...
  select(id, tipo, m17_date, fpp, fcf, conception, ga) %>%
  filter(!is.na(ga))
...
labels <- ga_fcf %>%
  filter(!between(ga, 6, 38) | !between(fcf, 115, 180)) %>%
  mutate(
    vjust = if_else(
      condition = fcf > 180,
      true = 1,
...
ga_fcf %>%
  ggplot() +
  geom_rect(
    aes(xmin = 6, xmax = 38, ymin = 115, ymax = 180),
