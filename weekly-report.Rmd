...
ga_fcf <- gt_emory_data %>%
  ...
  mutate(
    ...
    ga_group = case_when(
      ga < 9 ~ "Edad < 9s",
      between(ga, 9, 20) ~ "Edad >=9 y <=20 semanas",
      ga > 20 ~ "Edad > 20s"
    ),
    fcf_group = case_when(
      fcf < 120 ~ "FCF baja",
  ...
  select(
    id, m17_date, fpp, fcf, conception, ga,
    in_limits, study_group, ga_group, fcf_group
  ) %>%
  filter(!is.na(ga))

writexl::write_xlsx(ga_fcf, "output/m17_edad_y_frecuencia.xlsx")
...
```{r}
ga_fcf %>%
  group_by(study_group, ga_group, fcf_group) %>%
  summarize(value = n()) %>%
  bind_rows(
    ungroup(.) %>%
      mutate(study_group = "Total", ga_group = "Total") %>%
      group_by(study_group, ga_group, fcf_group) %>%
      summarize(value = sum(value)) %>%
      ungroup()
  ) %>%
  group_by(study_group, ga_group) %>%
  mutate(
    value = paste0(
  ...
  ungroup() %>%
  spread(fcf_group, value, fill = "") %>%
  mutate(
    study_group = if_else(
      condition = duplicated(study_group),
      true = "",
      false = study_group
    )
  ) %>%
