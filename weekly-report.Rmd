...
date_events %>%
  mutate(
    week = case_when(
      week <= 30 ~ "Semanas 1 - 30",
      week > 30 & week < max(week) ~ paste("Semanas 30 -", max(week)),
      week == max(week) ~ as.character(week)
    ),
    week = factor(
      week, levels = c(
        "Semanas 1 - 30",
        unique(week[grepl("Semanas 30", week)]),
  ...
  mutate_at(
    ...
    funs(
      if_else(
        condition = grepl("Semanas 1", week),
        true = "--",
        false = as.character(.)
      )
    )
...
classified %>%
  ...
  bind_rows(
    mutate(., week = factor("Todas", levels = levels(week)))
  ) %>%
  group_by(week) %>%
  mutate(
    dates = date %>%
...
classified %>%
  ...
  mutate(
    ...
    year = lubridate::year(as.Date(s4_date) + lubridate::days(3)),
  ...
  summarize(
    ...
    left_c = sum(left_c == "yes", na.rm = TRUE),
  ...
  left_join(
    subset(classified, select = "estudio.parto.c30_date") %>%
      ...
      mutate(
        year = lubridate::year(as.Date(born) + lubridate::days(3)),
        week = lubridate::week(as.Date(born) + lubridate::days(3)) - 1
      ) %>%
      group_by(year, week) %>%
      summarize(
        born = n()
...
```{r}
classified %>%
  select(study_id, enrolled, matches("^left"), matches("e3_date")) %>%
  filter(enrolled == "yes") %>%
  ...
  count(week, type) %>%
  complete(week, type = c("m", "o", "c"), fill = list(n = 0)) %>%
  spread(type, n, fill = 0) %>%
  rbind(
    summarize_at(
      .,
