...
#agregar variables de anotaciones h41
QC_completitud_ECM<-QC_completitud_ECM %>% filter(toupper(substr(id_filtro,1,3))!="3V3") %>% left_join(
dt_h41 %>% select(id=record_id, h41_c_stop_v2,h41_notes_v2, redcap_event_name ) %>% 
  filter(!is.na(h41_c_stop_v2) &  redcap_event_name=="year4_q1_48m_arm_1") %>% transmute(
    Participant_ID=id, 
    Monitor_apagado=recode(
      h41_c_stop_v2,"1"="Si", "0"="No"
      
    ),
    Notas_h41=h41_notes_v2
    )
)

# IMPLEMENTAR QC PARA UPAS EN B7
#leer rds de upas b7
upas_b7<-readRDS("C:/GitHub_repo/qc_exposure_hapin/rds/rds_hapin_upas.rds")
#seleccionar variables necesarias para QC
upas_b7_clean<-upas_b7 %>% group_by(
  ORIGEN, id_upas=head_UPASserial, id_hogar=head_SampleName, id_filtro=head_CartridgeID
) %>% summarize(
  min_date=min(as.Date(DateTimeLocal)),
  max_date=max(as.Date(DateTimeLocal))
)

#corregir id_upas
upas_b7_clean$id_upas <- sub("^PSP0+", "PSP", upas_b7_clean$id_upas)
upas_b7_clean<-upas_b7_clean %>% mutate(
  id_upas=if_else(
    str_length(id_upas)=="5",paste0(substr(id_upas,1,3),"000",substr(id_upas,4,5)),
    paste0(substr(id_upas,1,3),"00",substr(id_upas,4,6)),
  ),
  id_filtro=substr(id_filtro, 1,7)
)



# LB Crear las nuevas columnas para extraer id upas, id filtro y fecha del nombre del archivo UPAS
upas_b7_clean$upas_file_name <- str_extract(upas_b7_clean$ORIGEN, "PSP\\d+")
upas_b7_clean$filtro_file_name <- str_extract(upas_b7_clean$ORIGEN, "3V\\d+")
upas_b7_clean$fecha_file_name <- str_extract(upas_b7_clean$ORIGEN, "\\d{4}-\\d{2}-\\d{2}")
upas_b7_clean$id_hogar_file <- str_extract(upas_b7_clean$ORIGEN, "_(\\d{5})")
upas_b7_clean<-upas_b7_clean %>% mutate(
  id_hogar_file=substr(id_hogar_file,2,6)
)

QC_id_filtro_file<-upas_b7_clean %>% mutate(
  flag1=if_else(id_filtro!=filtro_file_name,"Id de filtro no coincide en nombre de archivo y contenido de archivo upas", NA_character_
) 
)%>% filter(!is.na(flag1))

QC_id_upas_file<-upas_b7_clean %>% mutate(
  flag2=if_else(id_upas!=upas_file_name,"Id de upas no coincide con el nombre de archivo y contenido de archivo upas", NA_character_
  ) 
)%>% filter(!is.na(flag2))

QC_id_nombre_archivo<-upas_b7_clean %>% mutate(
   flag3=if_else(
    id_hogar!=id_hogar_file, "ID de hogar no coincide con el nombre del archivo y el contenido del archivo",NA_character_
  )  
) %>% ungroup() %>%  filter(!is.na(flag3)) %>% transmute(
  id_hogar_en_archivo=id_hogar, id_hogar_nombre_archivo=id_hogar_file, id_upas, id_filtro, issue=flag3, solucion=""
)

QC_datos_redcap_upas<-dt_h41_b7 %>% filter(substr(id_filtro_h41,1,3)=="3V3")%>% transmute(
  id, fecha_h41=h41_date_v2, id_upas=paste0("PSP00",substr(id_ecm_h41,6,8)), id_filtro=id_filtro_h41
) %>% left_join(
  upas_b7_clean %>% ungroup() %>% transmute(id=id_hogar, id_upas, flag="Si")
) %>% filter(is.na(flag)) %>% left_join(
dt_h41 %>% select(id=record_id, h41_c_stop_v2,h41_notes_v2, redcap_event_name ) %>% 
  filter(!is.na(h41_c_stop_v2) &  redcap_event_name=="year4_q1_48m_arm_1") %>% transmute(
    id, 
    Monitor_apagado=recode(
  ...
  issue="No coinciden datos de redcap con datos del archivo, verificar id_upas, id_archivo y id_hogar",
  solucion=""
)

# h2a_upas_filtros_lb<-h2a_exposicion %>% transmute(id_tamizaje,  visita=substr( redcap_event_name,14,15), fecha=h2a_date,id_upas=h2a_id_upas1_per, id_filtro=h2a_id_filter1_per) %>% 
#   filter(!is.na(id_upas)) %>% bind_rows(
#     h2a_exposicion %>% transmute(id_tamizaje,  visita=substr( redcap_event_name,14,15), fecha=h2a_date,id_upas=h2a_id_upas2_per, id_filtro=h2a_id_filter2_per) %>%  filter(!is.na(id_upas))
#   ) 
# 
# #validar que filtro y upas coincida en redcap y en archivos upas
# h2a_upas_filtros_lb %>% left_join(
#   upas_linea_basal_clean  %>% ungroup() %>% transmute(id_upas, id_hogar, id_filtro, fecha_file_name, tiene_archivo="Si")
# ) %>% filter(is.na(tiene_archivo)) %>%filter(visita=="lb") %>%   group_by(visita) %>% count()
# 
...
list(
 ...
 "QC_id_hogar_upas"=QC_id_nombre_archivo,
 "QC_Redcap_UpasFile"=QC_datos_redcap_upas
