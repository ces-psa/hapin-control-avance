...
z10_vars <- c(
  ...
  "type_goelocation", "z10_gps_lat_new", "z10_gps_long_2", 
  "z10_gps_alt_2", "z10_gps_date_update", "z10_refer_location_new"
)
...
private_vars <- c(
  ...
  "z10_cell_participant", "z10_cell_hh", "z10_cell_other","z10_child_name",
  "z10_village", "z10_address", "referencia_ubicacion",
  "z10_sector_new","z10_addres_new", "z10_cell_new", "z10_gps_lat_new", "z10_gps_long_2", 
  "z10_gps_alt_2", "z10_gps_date_update", "z10_refer_location_new"
)
...
gt_z10 <- DBI::dbGetQuery(
  ...
  as_tibble() %>%
  mutate_at(
    vars(com_jalapa, com_jalapa_new),
    list(
      ~ recode(
        .,
        "2101308" = "SANTA CRUZ",
  ...
  mutate(
    community = if_else(
      condition = !is.na(com_jalapa_new) & com_jalapa_new != com_jalapa,
      true = com_jalapa_new,
      false = com_jalapa
    )
  )
...
gt_z10 <- gt_z10 %>%
  mutate(
    record_id = if_else(
      grepl("g", id_estudio, ignore.case = TRUE),
      id_estudio,
      record_id
    ),
    id_estudio = if_else(
...
z10_coordinates_new<-gt_z10 %>% select(id_estudio, record_id,
                                       z10_gps_lat_new, z10_gps_long_2, z10_gps_alt_2, z10_gps_date_update,
                                       z10_refer_location_new)%>% filter(!is.na(z10_gps_lat_new))

z10_dob<-gt_z10 %>% select(id_estudio, record_id, fecha_nacimiento_bb)
#------------------------------------------------------------------------------*
# Generate participant lookup table ----
#------------------------------------------------------------------------------*

#consentidas en hapin II
consentimientos_hapin_II<-gt_hapin_II_data %>% filter(redcap_event_name=="24_month_arm_1") %>%   filter(!is.na(s4_date)) %>% 
  select(id_estudio=id,s4_date,s4_consent, s4_consent_c, s4_mcon_date, s4_owa, s4_ocon_date) %>% 
  mutate(HAPIN_II_madre=if_else(
    s4_consent=="1", as.character(s4_mcon_date), NA_character_
  ),
  HAPIN_II_nino=if_else(
    s4_consent_c=="1", as.character(s4_mcon_date), NA_character_
  ),
  HAPIN_II_adulta=if_else(
    s4_owa=="1", as.character(s4_ocon_date), NA_character_
  )
    
  ) %>% filter(id_estudio!="99999")
...
gt_z10 %>%
  ...
  mutate_at(
    vars(
      z10_p_name, z10_p_lastname_2,
      z10_aow_name, z10_aow_lastname,
      z10_hh_name, z10_hh_lastname_2, z10_child_name,
    ),
    list(
      ~ if_else(
        condition = is.na(.),
  ...
  left_join(
    gt_emory_data_arm2 %>%
      filter(!is.na(e2_date) | !is.na(e1_date)) %>%
      select(id_estudio = id, visit, matches("^e[2345]_(date|by)$")) %>%
      mutate_at(vars(matches("e[2345]")), list(as.character)) %>%
      gather(variable, value, matches("e[12345]"), na.rm = TRUE) %>%
      separate(variable, into = c("crf", "variable")) %>%
      spread(variable, value) %>%
      mutate(value = paste(date, by)) %>%
      select(-by, -date) %>%
      complete(crf, visit = c("libres1", "libres2", "libres3","libres4", "libres5")) %>%
      mutate(
        value = paste(crf, sub("libres([0-9]+)", "\\1 - ", visit), value),
        visit = paste(crf, "en", sub("libres", "Segun sea necesario ", visit))
      ) %>%
      select(-crf) %>%
      spread(visit, value) %>%
      filter(!is.na(id_estudio), id_estudio != "99999")
  ) %>%
  left_join(
    gt_emory_data_arm2 %>% 
      filter(!is.na(e3_date)) %>% select(
        id_estudio=id, e3_date, e3_by
      ) %>% mutate(
        e3_madre=paste0("e3_madre - ",e3_date, " ", e3_by)
      ) %>% 
      select(id_estudio, e3_madre) %>% 
        filter(!is.na(id_estudio), id_estudio != "99999")
  ) %>% 
  left_join(
    gt_emory_data_arm2 %>% 
      filter(!is.na(e3_date_o)) %>% select(
        id_estudio=id, e3_date_o, e3_by_o
      ) %>% mutate(
        e3_owa=paste0("e3_owa - ",e3_date_o, " ", e3_by_o)
      ) %>% 
      select(id_estudio, e3_owa) %>% 
      filter(!is.na(id_estudio), id_estudio != "99999")
  ) %>% 
  left_join(
    gt_emory_data_arm2 %>% 
      filter(!is.na(e3_date_c)) %>% select(
        id_estudio=id, e3_date_c, e3_by_c
      ) %>% mutate(
        e3_nino=paste0("e3_nino - ",e3_date_c, " ", e3_by_c)
      ) %>% 
      select(id_estudio, e3_nino) %>% 
      filter(!is.na(id_estudio), id_estudio != "99999")
  )  %>% left_join(
    consentimientos_hapin_II %>% select(id_estudio, HAPIN_II_madre, HAPIN_II_nino,HAPIN_II_adulta)
  ) %>% 
  select(
    ...
    "Comunidad embarazada (nueva)" = com_jalapa_new,
    "Comunidad embarazada (original z10)" = com_jalapa,
    ...
    "Nombre del bebe"= z10_child_name,
...
#DATOS COMPLETOS DE LAS PARTICIPANTES
datos_participantes<-gt_z10 %>%
  mutate_all(
    list(~ iconv(., from = "Latin1", to = "UTF-8"))
  ) %>%
  mutate_at(
  ...
  left_join(
    gt_emory_data_arm2 %>%
      filter(!is.na(e2_date) | !is.na(e1_date) | !is.na(e3_date)) %>%
      select(id_estudio = id, visit, matches("^e[12345]_(date|by)$")) %>%
      mutate_at(vars(matches("e[12345]")), list(as.character)) %>%
      gather(variable, value, matches("e[12345]"), na.rm = TRUE) %>%
      separate(variable, into = c("crf", "variable")) %>%
      spread(variable, value) %>%
      mutate(value = paste(date, by)) %>%
  ...
  select(
    ...
    "Celular embarazada" = z10_cell_participant,
    "Celular esposo" = z10_cell_hh,
    "Celular_otro_miembro"=z10_cell_other,
    "DPI embarazada" = z10_dpi,
    "Fecha nacimiento embarazada" = z10_dob,
    "Nombre esposo" = esposo,
    "Fecha esperada de parto" = fecha_parto,
    "Fecha del parto" = fecha_nacimiento_bb,
    "Nombre otra adulta" = adulta,
    "Nombre_bb"=z10_child_name,
    HAPIN_II_madre,
    HAPIN_II_nino,
    HAPIN_II_adulta,
    matches("e[2345]"),
    sector_old=z10_village,
    direccion_old=z10_address,
    referencia_old=referencia_ubicacion,
    sector_new=z10_sector_new,
    referencia_new=z10_refer_location_new,
    direccion_new=z10_addres_new, 
    telefono_new=z10_cell_new
  )
