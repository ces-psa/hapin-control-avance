...
salidas<-arm2 %>% select(id,e3_date, e3_by) %>% filter(!is.na(e3_date)) %>% 
  #unir con los e3 de owa
  left_join(
    arm2 %>% select(id,e3_date_o, e3_by_o) %>% filter(!is.na(e3_date_o))
  ) %>% # unir con los e3 de niños
  left_join(
    arm2 %>% select(id,e3_date_c, e3_by_c) %>% filter(!is.na(e3_date_c))
  ) %>% #verificar nacimientos
  left_join(
    arm2 %>% select(id, c30_date) %>% filter(!is.na(c30_date))
...
salidas_detalle<-arm2 %>% select(id,e3_date, e3_by,e3_reason) %>% filter(!is.na(e3_date)) %>% 
  left_join(
    arm2 %>% select(id,e3_date_o, e3_by_o,e3_reason_o) %>% filter(!is.na(e3_date_o))
  ) %>%left_join(
    arm2 %>% select(id,e3_date_c, e3_by_c, e3_reason_c) %>% filter(!is.na(e3_date_c))
  ) %>% left_join(
    arm2 %>% select(id, c30_date) %>% filter(!is.na(c30_date))
  ) %>%mutate(
    type = if_else(
