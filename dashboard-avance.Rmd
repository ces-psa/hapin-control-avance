...
inscritas <- gt_emory_data %>%
  ...
  left_join(
    gt_emory_data %>%
      ...
      select(
        screening_id = id, fpp = m17_ga, screening_date = s1_date
      )
  ) %>%
  select(
    rowname, screening_id, id, redcap_event_name, everything(),
    -matches("^(s1|s2|m17|s3|s4)_"), fpp
  )
...
mapa_inscritas <- inscritas %>%
  ...
  select(id, screening_id, fpp, everything())
...
all_events %>%
  ...
  select(
    ...
    "FPP" = fpp,
    "Edad gestacional en dias" = current_days_pregnancy,
    "Evento" = event_name
  ) %>%
  tabla_interactiva()
```
