...
g_equipo_semana<- rec_semana %>% bind_rows(
 ...
 aes(x=semana, y=media, color=equipo)
  ...
  labs(
    title = "MEDIA DE PRODUCTIVIDAD POR SEAMANA EPIDEMIOLOGICA",
...
t_equipo_semana<-rec_semana %>% bind_rows(
  exp_semana
) %>% mutate(
  ...
  media_exposicion=if_else(equipo=="Exposicion",media, NA_real_),
        ...
        align = "l") %>% 
    kable_classic(full_width=F, html_font = "Cambria") %>% column_spec(1, width = "2cm",  bold = T) 



plot_grid(g_equipo_semana, g_equipo_mes, nrow = 2, align = "h")
