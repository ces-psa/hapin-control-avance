...
#Revisar los H51
gt_emory_repeat_data %>% filter(!is.na(h51_date)) %>%  select(record_id, h51_hhid, h51_date, h51_by) %>% left_join(
  comunidades %>% select(h51_hhid=id_estudio, Comunidad=community)
...
file.rename(
  from = "h51_enviados.html",
  to = "output/reference/h51_enviados.html"
)
