...
source("scripts/0_get_participants.R")
#sacar listado de participantes y edad del bb
gt_emory_data_arm2 %>% filter(!is.na(s6_arm)) %>% select(id, brazo=s6_arm) %>% 
  left_join(
    ...
    participando=case_when(
      ...
      type=="owa" & !is.na(e3_date) ~ "solo adulta",
      type=="owa" & is.na(e3_date) & is.na(e3_date_o) ~ "madre y adulta",
      type=="pwg" ~ "solo madre"
    ),
    edad_bebe=(Sys.Date() - as.Date(fecha_nacimiento))/30.25
  ) %>% select(id, id_tamizaje, fecha_nacimiento, edad_bebe, type, participando) %>% left_join(
    datos_participantes %>% select(id=`ID estudio`, `Nombre embarazada`, `Nombre otra adulta`, `Celular embarazada`, `Celular esposo`, 
                                   Celular_otro_miembro)
  ) %>% arrange(desc(edad_bebe)) %>% writexl::write_xlsx("output/Listado_llamadas_v1_covid_19.xlsx")
