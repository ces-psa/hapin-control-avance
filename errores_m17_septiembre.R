...
datos_m17 <- gt_emory_data_arm1 %>%
  filter(!is.na(m17_date), !is.na(s4_date)) %>% mutate(screening_id=id) %>% 
  # select(
  ...
  transmute(
    ...
    # edad gestacional segun fecha de ultima regla
    rc_lmp_ga = m17_lmp_ga %>%
      strsplit(split = "[.]") %>%
      map_dbl(~ sum(as.numeric(.x[1]), as.numeric(.x[2])/7, na.rm = TRUE)),
    ...
    fpp_lmp = m17_date - (ga_lmp * 7) + 280,
    fpp_obus = m17_date - (ga_obus * 7) + 280,
    ...
    fpp_esperada = m17_date - (ga_esperada * 7) + 280,
...
errores_m17 <- datos_m17 %>%
  ...
  mutate(
    ...
    problemas = data_frame(
      ...
      j = if_else(abs(ga_lmp - rc_lmp_ga) > 0, "error en fur", NA_character_),
    ...
    prioridad = data_frame(
      ...
      j = if_else(abs(ga_lmp - rc_lmp_ga) > 0, 2, 0),
...
#leer los ids revisados
revisados<-read_csv("data/m17_revisados_sep.csv")
revisados

errores_m17 %>% left_join(
  revisados %>% transmute(screening_id, revisado="Si", comentarios=`comentarios de revision`)
)%>% 
  filter(prioridad > 0) %>%write_csv(path = paste0("output/errors/m17_", Sys.Date(), ".csv"))

errores_m17 %>% anti_join(
  revisados %>% select("screening_id"=screenin_id)
) %>% 
  filter(prioridad > 0) %>%
