...
all_a23_solo_adulta <- gt_emory_data_arm2 %>%mutate(type = if_else(
  ...
  filter(!is.na(c30_date)) %>%
  transmute(id, dob = as.Date(c30_dob), visit="b1") %>% left_join(
...
c31_7_dias<-all_c33 %>% filter(expected) %>% transmute(house_id=id, fecha_nacimiento=dob, fecha_visita=visit_start+13, tipo_visita=visit, limite_inferior=visit_start, 
